...
```{r}
partition <- createDataPartition(data_covid$deaths_per1000, p = .8, list = F)
data_covid_train_c <- data_covid %>% slice(partition)
data_covid_test_c <- data_covid %>% slice(-partition)
```

## Preprocess

```{r}
data_covid_prep <- preProcess(x = data_covid_train_c, method = c("center", "scale"))
data_covid_train_c_processed <- predict(data_covid_prep, data_covid_train_c)
data_covid_test_c_processed <- predict(data_covid_prep, data_covid_test_c)
```
...
```{r}
death_model <- lm(deaths_per1000 ~ Poverty + Unemployment + Black + Hispanic + IncomePerCap, data=data_covid_train_c_processed)
summary(death_model)
```
```{r}
