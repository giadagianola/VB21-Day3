...
data_covid_train <- data_covid %>% slice(partition)
data_covid_test <- data_covid %>% slice(-partition)
```

## Preprocess

```{r}
data_covid_prep <- preProcess(x = data_covid_train, method = c("center", "scale"))
data_covid_train_processed <- predict(data_covid_prep, data_covid_train)
data_covid_test_processed <- predict(data_covid_prep, data_covid_test)
```
...
```{r}
death_model <- lm(deaths_per1000 ~ Poverty + Unemployment + Black + Hispanic + IncomePerCap, data=data_covid_train_processed)
summary(death_model)
```
```{r}
